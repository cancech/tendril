...
import tendril.bean.requirement.RequiresOneOfProp;
...
public abstract class AbstractRecipeGenerator<CREATOR extends JBase> {
	...
	/**
	 * Populate the builder with the specifics of what the recipe requires. The details of the class will be applied to the builder before this is called, so this only needs to focus on the internals
	 * of the recipe.
	 * 
	...
	/**
	 * Get the recipe class that is to be employed for the indicated bean. By default this will make the determination based on what annotation is applied to the creator class (i.e.: what class this
	 * is a recipe for).
	 * 
	...
	protected void generateRecipeRequirements(ClassBuilder builder) {
		...
		builder.buildMethod("setupEnvironmentRequirement").addAnnotation(JAnnotationFactory.create(Override.class)).setVisibility(VisibilityType.PROTECTED).buildParameter(recipeClass, "requirement")
				.finish().addCode(wrapLines(getRequirementLines(creator, RequiresEnv.class, RequiresOneOfEnv.class, RequiresNotEnv.class), "requirement.", ";")).finish();

		// Configuration for property requirements
		builder.buildMethod("setupPropertyRequirement").addAnnotation(JAnnotationFactory.create(Override.class)).setVisibility(VisibilityType.PROTECTED).buildParameter(recipeClass, "requirement")
				.finish().addCode(wrapLines(getRequirementLines(creator, RequiresProp.class, RequiresOneOfProp.class, RequiresNotProp.class), "requirement.", ";")).finish();
	}
	 ...
	 * @param requiresAnnontation      {@link Class} extending {@link Annotation} representing the annotation for all required (AND)
	 * @param requiresOneOfAnnontation {@link Class} extending {@link Annotation} representing the annotation for one of required (OR)
	 * @param requiresNotAnnontation   {@link Class} extending {@link Annotation} representing the annotation for none (NOT)
	 */
	private List<String> getRequirementLines(JBase element, Class<? extends Annotation> requiresAnnontation, Class<? extends Annotation> requiresOneOfAnnontation,
			Class<? extends Annotation> requiresNotAnnontation) {
		List<String> lines = new ArrayList<>();

		// Account for any required environments
		populateReqs(lines, element, requiresAnnontation, "addRequired", true);
		populateReqs(lines, element, requiresOneOfAnnontation, "addRequiredOneOf", false);
		populateReqs(lines, element, requiresNotAnnontation, "addRequiredNot", true);
		return lines;
	}

	/**
	 * Generate the code to account for requirements for the element
	 * 
	 ...
	 */
	private void populateReqs(List<String> lines, JBase element, Class<? extends Annotation> annotation, String methodName, boolean individually) {
