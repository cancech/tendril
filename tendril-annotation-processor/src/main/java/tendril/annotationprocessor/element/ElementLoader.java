...
public abstract class ElementLoader {
    // TODO refactor cache into a separate class?
    /** Cache to save/store processed element, to avoid needing to reprocess them in the future */
    private static final Map<String, JBase> cache = new HashMap<>();
     ...
     */
    public static JClass loadClassDetails(TypeElement element) throws MissingAnnotationException {
     ...
     */
    private static String classNameKey(TypeElement element) {
        return element.toString();
    }

    /**
     * Generate the cache key when caching a method
     * 
     * @param enclosingClass {@link TypeElement} which contains the method
     * @param element {@link ExecutableElement} which represents the method
     * @return {@link String} the key
     */
    private static String methodNameKey(TypeElement enclosingClass, ExecutableElement element) {
        return classNameKey(enclosingClass) + "::" + element.toString();
    }

    /**
     * Generate the cache key when caching a field
     * 
     * @param enclosingClass {@link TypeElement} which contains the field
     * @param element {@link VariableElement} which represents the field
     * @return {@link String} the key
     */
    private static String fieldNameKey(TypeElement enclosingClass, VariableElement element) {
        return classNameKey(enclosingClass) + "::" + element.toString();
    }
     ...
     */
    public static Pair<JClass, JMethod<?>> loadMethodDetails(ExecutableElement element) throws MissingAnnotationException {
        TypeElement enclosingClass = (TypeElement) element.getEnclosingElement();
        return Pair.of(loadClassDetails(enclosingClass), (JMethod<?>)cache.get(methodNameKey(enclosingClass, element)));
    }
     ...
     * @throws MissingAnnotationException if attempting to load a class which is making use of an annotation that does not (yet) exist
     */
    private static void loadExecutableElementDetails(NestedClassMethodElementBuilder<?, ?, ?> builder, TypeElement enclosingClass, ExecutableElement element) throws MissingAnnotationException {
        ...
        cache.put(methodNameKey(enclosingClass, element), builder.build());
    }
     ...
     * @throws MissingAnnotationException if attempting to load a class which is making use of an annotation that does not (yet) exist
     */
    private static void loadFieldDetails(FieldBuilder<?> builder, TypeElement enclosingClass, VariableElement varElement) throws MissingAnnotationException {
        loadElementFinality(builder, varElement);
        loadElementMods(builder, varElement);
        loadAnnotations(builder, varElement);
        cache.put(fieldNameKey(enclosingClass, varElement), builder.build());
    }
     ...
     * @throws MissingAnnotationException if attempting to load a class which is making use of an annotation that does not (yet) exist
     */
    private static void loadAnnotations(BaseBuilder<?, ?> builder, Element element) throws MissingAnnotationException {
     ...
     */
    private static void applyAnnotatonByClass(BaseBuilder<?, ?> builder, ClassType annonType, TypeElement annotationElement, Element appliedTo) throws ClassNotFoundException {
     ...
     * @throws MissingAnnotationException if attempting to load a class which is making use of an annotation that does not (yet) exist
     */
    private static void applyGeneratedAnnotation(BaseBuilder<?, ?> builder, ClassType annonType, AnnotationMirror annonMirror, Element appliedTo) throws MissingAnnotationException {
